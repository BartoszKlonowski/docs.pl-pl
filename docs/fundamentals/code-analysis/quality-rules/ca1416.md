...
Ta reguła jest domyślnie włączona tylko dla projektów przeznaczonych dla platformy .NET 5,0 lub nowszej. Można go jednak [włączyć](#configurability) dla projektów przeznaczonych dla innych platform.
...
Aby uzyskać więcej informacji, zobacz [Opcje konfiguracji reguł jakości kodu](../code-quality-rule-options.md).
