...
Ta zasada jest wyzwalana, gdy oba poniższe warunki *mogą być* spełnione dla wystąpienia [Newtonsoft.Json.Jsonserializer](https://www.newtonsoft.com/json/help/html/T_Newtonsoft_Json_JsonSerializer.htm) , które zostało przesłane do metody deserializacji lub zainicjowane jako pole lub właściwość:
...
Ta reguła jest podobna do [CA2329](ca2329.md), ale w tym przypadku analiza nie może ostatecznie określić, czy serializator jest skonfigurowany niezabezpieczony.
