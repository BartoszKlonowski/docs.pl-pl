...
---
# <a name="breaking-changes-in-net-core-30"></a>Istotne zmiany w programie .NET Core 3,0

W przypadku migrowania do wersji 3,0 programu .NET Core, ASP.NET Core lub EF Core, istotne zmiany wymienione w tym artykule mogą mieć wpływ na aplikację.
...
- [Przekazanie grupy GroupCollection do metod rozszerzających, które \<T> wymagają uściślania](#passing-groupcollection-to-extension-methods-taking-ienumerablet-requires-disambiguation)
...
- [CryptoStream. Dispose przekształca końcowy blok tylko podczas pisania](#cryptostreamdispose-transforms-final-block-only-when-writing)
